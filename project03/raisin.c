                              ...
                              int address)
{
  ...
  if (check_invalid_opcode(opcode) == 1 ||
      (uses_register(opcode, 1) && check_invalid_reg(reg_one)) ||
      (uses_register(opcode, 2) && check_invalid_reg(reg_two)) ||
      (uses_memory_address(opcode) && address % 4 != 0))
    return 1;

  /* checks for special registers r16 and r17 paired with opcodes that modify
  the first register */
  if ((opcode == 2 || opcode == 3 || opcode == 4 || opcode == 5 ||
       opcode == 6 || opcode == 7 || opcode == 8 || opcode == 9 ||
       opcode == 10 || opcode == 17 || opcode == 18 || opcode == 20) &&
      (reg_one == 16 || reg_one == 17))
    return 1;
...
int uses_register(int opcode, int register_num)
{
  ...
  if (register_num == 2 && opcode != 20 && opcode != 19 && opcode != 18 &&
      opcode != 9 && opcode != 10)
    return 1;
